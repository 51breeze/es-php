...
class Context extends BaseContext{
    ...
    isModuleNeedImport(module){
        if(this.checkModuleImportExclude(module)){
            return false;
        }
        let isVM = this.isVModule(module);
        if(isVM){
           return true;
        }
        ...
        if(module.isDeclaratorModule){
            return !this.isPhpDepModule(module)
        }
        return true;
    }

    isPhpDepModule(module){
    ...
    getAccessorName(name, desc, accessor='get'){
        if(Utils.isStack(desc) && Utils.isModule(desc.module)){
            ...
            resolveName = _name;
            if(!module.isDeclaratorModule){
                const scope = module.compilation.pluginScopes.scope;
                while(true){
                    const records = isStatic ? module.getMethod(resolveName) : module.getMember(resolveName);
                    if(!records)break;
                    if(records.module && records.module.compilation.pluginScopes.scope === scope){
    ...
    getModuleMappingFolder(module){
        ...
        let isCom = !(isRM || isVM) ? Utils.isCompilation(module) : false;
        if(!(isRM || isVM || isCom))return null;
        if(isVM){
        ...
        let nonGlobal = false;
        if(isRM && !isVM){
            nonGlobal = module.getStacks().some(stack=>{
                const compilation =stack.compilation;
                return compilation && compilation.pluginScopes.scope ==='es-php';
            })
        }
        if(!nonGlobal){
            ...
            }else if(isCom && module.isGlobalFlag){
                if(module.pluginScopes?.scope ==='global'){
                    file += '.global';
                }
            }
        }
