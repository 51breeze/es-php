...
import {createUniqueHashId, parseUrlAnnotation, parseReadfileAnnotation, getSourceAnnotations, annotationIndexers, parseRouteAnnotation} from "@easescript/transform/lib/core/Common";
...
function createGetAssetsContentNode(ctx, stack, resolveFile, local=null, graph=null, context = null){
    ...
    if(source){
        ...
        if(asset){
            if(!graph)graph=ctx.getBuildGraph();
...
function createReadfileAnnotationNode(ctx, annot){
            ...
            ctx.addHook(async()=>{
                ...
                if(compilation){
                    ...
                    if(compilation.mainModule){
                        ...
                        let source = ctx.getModuleImportSource(compilation.file, annot.file);
    ...
    const make = (list)=>{
        return list.map( object=>{
            ...
            if(object.content){
                ...
                properties.push(ctx.createProperty(ctx.createIdentifier('content'),contentNode))
            }
            if(object.children){
                properties.push(ctx.createProperty(ctx.createIdentifier('children'),ctx.createArrayExpression(make(object.children, false))))
            }
...
function canUseNullCoalescingOperator(stack){
    ...
    if(parentStack.isUpdateExpression || parentStack.isUnaryExpression)return false;
