...
function createParamNodes(ctx, stack, params){
   ...
   const items = params.map( (item,index)=>{
      if( item.isObjectPattern ){
         let sName = ctx.getLocalRefName(stack, '_s');
         before.push(
            createInitNode(
               ctx, 
               sName,  
         ...
         const sName = ctx.getLocalRefName(stack,'_s');
         before.push(createInitNode(
            ctx, 
            sName,  
            ctx.createVarIdentifier(sName),  
...
export default function(ctx,stack,type){
   ...
   if( stack.expression && stack.expression.async || stack.async ){
      ...
      const content = ctx.createFunctionExpression(block, params);
      if(params.length>0){
      ...
      const executer = ctx.createFunctionExpression(tryNode, [
         ctx.createIdentifier('resolve'),
         ctx.createIdentifier('reject')
      ]);
