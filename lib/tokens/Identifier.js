...
import { createClassRefsNode , createStaticReferenceNode, createDocumentScopeId} from "../core/Common";
...
export default function(ctx,stack){
    ...
    if( isDeclarator ){
        if( desc.parentStack.isImportDeclaration && stack.compilation.mainModule){
            let resolve = desc.parentStack.getResolveFile();
            return ctx.createCallExpression( 
                createStaticReferenceNode(ctx, stack, 'System', 'getScopeVariable' ),
                [
                    ctx.createLiteral( createDocumentScopeId(stack.compilation, resolve) ),
