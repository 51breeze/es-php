...
import { createClassRefsNode,createComputedPropertyNode,createScopeIdNode,createStaticReferenceNode } from '../core/Common';
...
function MemberExpression(ctx,stack){
    ...
    if(objectType.isNamespace && !stack.parentStack.isMemberExpression){
        const mappingNs = ctx.getMappingNamespace( stack.value() );
        if( mappingNs !== null ){
            return mappingNs ? ctx.createIdentifier( mappingNs +'\\'+ stack.property.value(), stack.property) : ctx.createToken(stack.property);
        }
